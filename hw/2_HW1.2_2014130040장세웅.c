...
void _EXTI_Init(void)
{
	...
	//인터럽트 표 참고 77이 무슨 의미인지? -> EXTI8,9 레지스터 의미
...
void TIM5_IRQHandler(void)  // 100ms Interrupt
{
    ...
    else if((stop2 == 0) && (toggle == 1))// 홀수 번째 (SW6) 키를 눌렀을 때
    {
      StopWatch_System();// StopWatch_System 함수 동작
    }
    else if((stop2 == 0) && (toggle == 0)) // 짝수 번째 (SW6) 키를 눌렀을 때
    {
      //시간 멈춤
    }
}
...
void Watch_System()
{
    ...
    {	
      ...
      S1++;  // 1초시계 1씩 증가 시계
            ...
            if(S1 == 10) // 1초 시계가 10이면
            {
                ...
                if(S10 == 6) // 10초시계가 6이 되면 0으로 : why? -> 60초이면 1분이므로 6부분이 0으로 되어야한다.
                {
					...
					M1++;
                 	...
                 	if(M1 == 10) // 1분 시계가 10이면
                 	{
	                	 M1 = 0; // 1분시계를 0으로
	                	 M10++; // 10분 시계를 1씩 증가
	                	 
	                	 if(M10 == 6) // 10분시계가 6이 되면 0으로 : why? -> 60분이면 1시간이므로 6부분이 0으로 되어야한다.
	                	 {
	                		M10 = 0; //10분시계를 0으로
	                		H1++; //1의자리 시간 증가
	                		
	                		if(H1 == 10)  //1의 자리 시간 10이면
	                		{
	                			H1 = 0; //1의자리 시간 0으로
	                			H10++; //10의 자리 시간 증가
	                			
	                			if(H10 == 2) //10의자리 시간이 2이면
	                			{
	                				H10 = 0; //10의자리 시간 0으로
                                }
        ...
        LCD_DisplayChar(1,12,S100M+0x30); //100ms 표기
        LCD_DisplayChar(1,3,H10+0x30); //10의자리 시간 표기
        LCD_DisplayChar(1,4,H1+0x30); //
        LCD_DisplayChar(1,6,M10+0x30);
        LCD_DisplayChar(1,7,M1+0x30);
        LCD_DisplayChar(1,9,S10+0x30);	
