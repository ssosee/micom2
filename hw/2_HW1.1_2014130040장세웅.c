...
void TIMER3_Init(void) //★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
{
	...
	TIM3->PSC = 4200-1;	// Prescaler 84,000,000Hz/4200 = 20,000 Hz (0.2ms)  (1~65536) //-1은 rule임
	TIM3->ARR = 1000-1;	// Auto reload  0.1ms * 10 = 1ms //반으로 줄이려면 10을 5로 //1초 주기로 만들고 싶으면 0.1ms * 10000= 1s
    //0.1ms * 1000= 100ms
...
void _EXTI_Init(void)
{
	...
	EXTI->FTSR |= 0x000300;		// Falling Trigger Enable  (EXTI8:PH8)
...
void TIM3_IRQHandler(void)  // 100ms Interrupt
{
    ...
    if(stop == 1) // stop 키를 눌렀을 때
    {
     
    }
	//LED_cnt++;	
    else if(stop == 0) // clear 키를 눌렀을 때
    {
...
{
	if(EXTI->PR & 0x0100) 		// EXTI8 nterrupt Pending? //Clear key
	{
	...
	else if(EXTI->PR & 0x0200) 	// EXTI9 Interrupt Pending? //Stop key
	{
...
void Watch_System()
{
  S100M++; //1/10초 시계 1씩 증가
    LCD_DisplayChar(1,3,S100M+0x30);
    if(S100M == 10) //1/10초 시계가 10이면
    {	
      S100M = 0; // 1/10초 시계 0으로 초기화
      S1++;  //1초시계 1씩 증가 시계
      LCD_DisplayChar(1,1,S1+0x30);
            if(S1 == 10) //1초 시계가 10이면
            {
                S1 = 0;
                S10++; //10초 시계 1씩 증가
                LCD_DisplayChar(1,0,S10+0x30);
